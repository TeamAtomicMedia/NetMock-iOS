...
private extension Document {
    init(request: Request, body: [DocumentStoreEntry.CapturedResponse]) {
    ...
    public func save(
        ...
        for document in self.documents {
            ...
            do {
                ...
                captureLogger.debug("Failed to create folder \(directory)")
                break
            }
            
            let url = directory.appendingPathComponent(filename).appendingPathExtension("nm")
            captureLogger.debug("Writing to \(url)")
            
            do {
                try data.write(to: url, options: [.atomic, .completeFileProtection])
                writeCount += 1
            } catch {
                captureLogger.debug("Failed to write document \(document.key): \(error)")
            }
        }
        ...
        if writeCount == documentCount {
            captureLogger.debug("All files successfully written")
        } else {
            captureLogger.debug("\(writeCount)/\(documentCount) Documents successfully written")
        }
    }
